...
<%
	...
	if (op.equals("delete")) {
		...
		if (op.equals("insert")) {
			...
			sql="INSERT INTO `scene`.`inspector` (`account`, `name`, `password`) VALUES ('"+account+"', '"+name+"', '"+pw1+"')";
			ret = 60;
		}
		else {
			if (op.equals("modify")) {
				sql = "select * from inspector where id='"+session.getAttribute("LoginId")+"' AND password='"+request.getParameter("oldPwd")+"'";
				rs = s.executeQuery(sql);
				if (!rs.next()) {
					response.sendRedirect("BenchPrompt.jsp?result=1");				
					return;
				}
				sql="update inspector set password='"+request.getParameter("newPwd")+"' where id='"+session.getAttribute("LoginId")+"'";
				ret = 90;
			}
			else {
				con.close();   
