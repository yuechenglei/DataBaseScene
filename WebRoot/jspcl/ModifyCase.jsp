...
<%
	...
	if (op.equals("delete")) {
		...
		if (op.equals("insert")) {
	sql = "INSERT INTO `scene`.`case` (`name`, `date`, `inspector`, `xkh`, `m`, `date_anfa`, `place`,`description`) VALUES ('"
			+ name
			+ "', '"
			+ date
			+ "', '"
	...
	id = request.getParameter("id");
	sql = "update `scene`.`case` set `name`='" + name
			+ "', `inspector`='" + inspector + "', `xkh`='"
			+ xkh + "', `m`='a', `date_anfa`='" + date_anfa
	...
	if (sql2 != "") {
		...
		response.sendRedirect("BenchPrompt.jsp?result=" + arg + "&id="
				+ id2);
		//用来插入案件成功后直接跳转到上传图片
	} else {
