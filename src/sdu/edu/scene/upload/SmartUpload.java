...
public class SmartUpload {
	...
	/*
	 * 公共接口类HttpServletRequest继承自ServletRequest.客户端浏览器发出的请求被封装成为一个HttpServletRequest
	 * 对象。所有的信息包括请求的地址，请求的参数，提交的数据，上传的文件客户端的ip甚至客户端操作系统都包含在其内。
	 * HttpServletResponse继承了ServletResponse接口，并提供了与Http协议有关的方法，这些方法的主要功能是设置
	 * HTTP状态码和管理Cookie
	 */
...
/*
			...
			if (flag3) {
				sdu.edu.scene.upload.File file = new sdu.edu.scene.upload.File();
				file.setParent(this);
	...
	public InputStream get_inputstream() {
		...
		InputStream is;
		is = new ByteArrayInputStream(m_binArray);
		return is;
