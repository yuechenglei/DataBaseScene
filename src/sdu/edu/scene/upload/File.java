...
public class File {
	...
	public InputStream getInputStream() {
		...
		long	l=0;
		InputStream	is;
		is = new ByteArrayInputStream(m_parent.m_binArray);
		try {
			l = is.available();
		}
		catch (IOException ioexception){}
		try {
			is.skip(m_startData);
			l = is.read(b, 0, m_size);
		} catch (IOException ioexception) {
			System.out.println("getInputStream() ioexception: m_startData="+m_startData+"            m_size="+m_size+"       length="+l);
		}
